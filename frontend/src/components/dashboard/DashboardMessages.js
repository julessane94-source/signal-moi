...
export default function DashboardMessages() {
  ...
  const fetchMessages = async () => {
    try {
      ...
      // Si l'endpoint n'existe pas, afficher un message friendly
      if (response.status === 404) {
        console.log('[MESSAGES] Endpoint /api/messages non disponible')
        setMessages([])
        setLoading(false)
        return
      }
      ...
      console.error('[MESSAGES] Erreur lors du chargement:', error)
      setMessages([])
      // Ne pas afficher l'erreur si c'est juste que l'endpoint n'existe pas
    } finally {
      setLoading(false)
    }
  }
